...
本仓库是**纯 Python 脚本 + SKILL.md 包装**，不依赖任何特定 AI 框架。
`globe_bs_pipeline.py` / `check_env.py` 只用到标准库 + GLOBE 自带的 PyAT + 独立 Python 的
matplotlib/netCDF4，任何能执行终端命令的 agent 都能直接调用。

- **Hermes**：把本目录放到 `~/.hermes/skills/data-science/globe-bs-pipeline/`，agent 会按 SKILL.md 自动触发
- **Claude Code**：放到 `~/.claude/skills/globe-bs-pipeline/`（SKILL.md 格式兼容，`triggers` 字段被忽略，无害）
- **Codex / Cursor / Aider / 其他**：无需安装，直接让 agent 在终端跑下面的命令即可
- **人工使用**：自己开终端跑同样的命令
